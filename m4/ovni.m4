...
AC_DEFUN([AC_CHECK_OVNI],
	[
		AC_ARG_WITH(
			[ovni],
			[AS_HELP_STRING([--with-ovni@<:@=DIR@:>@], [enable ovni instrumentation and specify the ovni library installation prefix])],
		...
		if test x"${ac_use_ovni}" = x"yes" ; then
			...
			ovni_CPPFLAGS="-DENABLE_OVNI_INSTRUMENTATION ${ovni_CPPFLAGS}"
		fi
