...
#pragma oss task
oss_coroutine timeout_task(int numWaits, uint64_t *timeouts, int timeout)
{
	for (int i = 0; i < numWaits; ++i) {
		co_await oss_co_wait_for(timeout, &(timeouts[i]));
	}
}
...
int main(int argc, char **argv)
{
	...
	for (int t = 0; t < numTasks; ++t) {
		timeout_task(numWaits, timeouts, timeout);
		timeouts += numWaits;
	}
	#pragma oss taskwait
