...
public:
	...
	TaskVerifier(
		int &id, type_t type, int *variable,
		Atomic<int> *numConcurrentTasks = 0,
		Atomic<int> *numConcurrentReductionTasks = 0
	) : _id(id++), _runsAfter(), _runsBefore(), _runsConcurrentlyWith(),
		_runsConcurrentlyWithReduction(), _status(NOT_STARTED), _type(type),
		_variable(variable), _numConcurrentTasks(numConcurrentTasks),
		_numConcurrentReductionTasks(numConcurrentReductionTasks)
	{
	}
...
int main(int argc, char **argv)
{
	...
	for (
		std::vector<std::pair<VerifierConstraintCalculator, std::string> >::const_iterator it = testConstraintCalculators.begin();
	...
	for (
		std::vector<Atomic<int> *>::const_iterator it = testCounters.begin();
		it != testCounters.end();
		it++
	) {
		delete *it;
	...
	for (
		std::vector<std::vector<TaskVerifier *> *>::const_iterator it = testVerifiers.begin();
		...
		for (
			std::vector<TaskVerifier *>::const_iterator vit = verifiers->begin();
			vit != verifiers->end();
			vit++
		) {
			TaskVerifier *verifier = *vit;
